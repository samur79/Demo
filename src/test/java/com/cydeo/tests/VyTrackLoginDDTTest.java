...
// The list in excel file at first and at the end is different. Pay attention to that
    //otherwise Eddie will fail
    @Test
    public void loginDDTTest() throws IOException {
        // we need to write these lines for reading from an excell file
        String filePath = "VyTrackQa2Users.xlsx";
        FileInputStream in = new FileInputStream(filePath); //which file we are going to open
        XSSFWorkbook workbook = new XSSFWorkbook(in); //open file
        XSSFSheet sheet = workbook.getSheet("data");// go to the sheet
